...
graphAD <- read_graph(file = '~/redesROSMAP/graphs/AD_ROSMAP_RNAseq_MutualInfograph_percentile99.99.graphml',
                    format = 'graphml')


graphnoAD <- read_graph(file = '~/redesROSMAP/graphs/noAD_ROSMAP_RNAseq_MutualInfograph_percentile99.99.graphml',
                      format = 'graphml')
...
hub_gene_AD <- data.frame(gene = names(hub_genes_AD), degree = hub_genes_AD)
...
hub_gene_AD <- merge(hub_gene_AD, traductions_hub_AD, by.x = "gene", by.y = "ensembl_gene_id", all.x = TRUE)
...
hub_genes_noAD <- data.frame(gene = names(hub_genes_noAD), degree = hub_genes_noAD)

# Convert ENSMBL a SYMBOL

traductions_hub_noAD <- convert_ens_to_symbol(hub_genes_noAD$gene)

# Merge gene names traduction

hub_genes_noAD <- merge(hub_genes_noAD, traductions_hub_noAD, by.x = "gene", by.y = "ensembl_gene_id", all.x = TRUE)
